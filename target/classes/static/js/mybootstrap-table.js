(function(root, factory, plug){
	...
	var pageContent = {
		...
		pageCount:30,//总共有多少页
		pageNow:10,//当前是第几页
		url:"http://localhost:8888/userLogin/getAllSession",
		...
		if(pageContent.url){
			...
			$.ajax({
				...
				error:function(jqXHR, data){
					//window.location = "http://localhost/userLogin/login.html";
					//alert(jqXHR.status);
					if(jqXHR.status === 403){
						alert("没有操作权限");
					}else{	
						alert("出现未预计错误 : "+ jqXHR.status);
						window.location = "http://localhost/userLogin/style.html";
					}
				}
			});
		}
